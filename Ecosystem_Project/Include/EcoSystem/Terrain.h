...
class Terrain {
 public:
  ...
  void Init(float _iga, float _igl, float _igh, unsigned _w, unsigned _h,
            float _grl, float _grh, float _gm, float _fm, float _frl,
            float _frh) noexcept;
  ...
  const Map<std::pair<float, float>>& GetFertilizerLayerThresh(
      void) const noexcept;
  ...
  // A*
  std::vector<GridPos> GetShortestPath(const GridPos& _source,
                                       const GridPos& _dest) noexcept;
  GridPos GetBestGrassPos(const GridPos& _source, float _mnLimit,
                          float _minAlpha) noexcept;
 ...
 private:
  ...
  std::vector<Node*> GetNeighbours(const GridPos& _src, const GridPos& _dest,
                                   float _curT, Node* _prev) noexcept;
  template <typename T>
...
inline void Terrain::Normalize(Map<T>& _m) {
  ...
  for (unsigned i = 0; i < _m.size(); ++i)
    for (unsigned j = 0; j < _m[i].size(); ++j)
      if (_m[i][j] > largest) largest = _m[i][j];

  for (unsigned i = 0; i < _m.size(); ++i)
    for (unsigned j = 0; j < _m[i].size(); ++j) _m[i][j] /= largest;
}
