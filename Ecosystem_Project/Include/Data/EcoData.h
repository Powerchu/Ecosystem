...
static const char* Spawnables[CREATURE_COUNT] = {"Rabbit 1", "Fox1"};
...
template <typename T,
          typename SFNAE = std::enable_if_t<std::is_base_of_v<Creature, T>, T>>
void SpawnCreature(unsigned short _x, unsigned short _y,
                   const EvolutionData& _evo, const Traits& _trait, int _i) {
  EcoSystem& eco = EcoSystem::GetInst();
  Creature* c =
      static_cast<Creature*>(new T{_trait, static_cast<unsigned>(_i)});
...
struct SpawnVisitor {
  SpawnVisitor(unsigned short _x, unsigned short _y, const EvolutionData& _evo,
               const Traits& _t) noexcept
      : mnGridX{_x}, mnGridY{_y}, mEvo{_evo}, mTrait{_t} {}

  template <typename T>
  SpawnVisitor(T _x, T _y, const EvolutionData& _evo, const Traits& _t) noexcept
      : SpawnVisitor{static_cast<unsigned short>(_x),
                     static_cast<unsigned short>(_y), _evo, _t} {}
...
struct VisitTuple_Impl {
  ...
  static void Visit(int _i, F _func) {
    if (_i == I - 1)
      _func.template operator()<std::tuple_element_t<I - 1, T>>(_i);
    else
...
void VisitSpawnTuple(F _func, int _i) {
  VisitTuple_Impl<std::tuple_size_v<CreatureList>>::Visit<CreatureList>(_i,
                                                                        _func);
}
}  // namespace Data
}  // namespace Ecosystem
