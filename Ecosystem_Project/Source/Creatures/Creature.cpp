...
Ecosystem::EvolutionData::EvolutionData(float _repT, float _repl, float _muta) noexcept
...
Ecosystem::EvolutionData& Ecosystem::EvolutionData::operator=(const EvolutionData& _evo) noexcept
{
	if (this != &_evo)
	{
...
Ecosystem::Creature::Creature(const std::string& _name, unsigned short _flags, const Traits& _t, unsigned _id) noexcept
	: mBitFlags{ _flags },
	mName{ _name }, mUniqueID{ RandomString(16) }, mnColorCode{}, mnChartID{ _id },
	mfFatigue{ std::make_pair(0.f,0.f) }, mfEnergy{ std::make_pair(0.f,0.f) },
	mfFatigueThresh{ 0.3f }, mfEnergyThresh{ 0.3f },
	mTraits{ _t }, mEvoData{},
	mCurPath{}, mfAccPathDt{},
	mnPosX{}, mnPosY{}, mnHomeX{}, mnHomeY{}
{
	SetColor();
}
...
void Ecosystem::Creature::GetGridPosition(unsigned& _outX, unsigned& _outY) const noexcept
{
	_outX = mnPosX;
	_outY = mnPosY;
}
...
void Ecosystem::Creature::GetHomeGridPosition(unsigned& _outX, unsigned& _outY) const noexcept
{
	_outX = mnHomeX;
	_outY = mnHomeY;
}
...
Ecosystem::GridPos Ecosystem::Creature::GetPendingDestination(void) const noexcept
{
	if (mCurPath.empty())
		return GridPos{ -1,-1 };
...
const Ecosystem::EvolutionData& Ecosystem::Creature::GetEvoData(void) const noexcept
{
	return mEvoData;
}
...
void Ecosystem::Creature::SetFatigueBase(const std::pair<float, float>& _minMax) noexcept
{
	mfFatigue = _minMax;
}

void Ecosystem::Creature::SetEnergyBase(const std::pair<float, float>& _minMax) noexcept
{
	mfEnergy = _minMax;
}
...
void Ecosystem::Creature::SetColor(float _red, float _green, float _blue, float _alpha ) noexcept
{
