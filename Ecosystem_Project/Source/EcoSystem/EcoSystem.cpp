...
Ecosystem::EcoSystem::EcoSystem(unsigned _w, unsigned _h, unsigned _s) noexcept
	: mnPeakPops{ 0 },
	mnLogWindow{ 20 }, mnWidth{ _w }, mnHeight{ _h }, mnScale{ _s },
	mnWindowX{ 0 }, mnWindowY{ 0 },
	mfDelta{ 0.016f }, mfTitleBarSize{ 0.f }, mfTimeStep{ 1.f }, mfLogFreq{ 1.f }, mfLogAccDt{ 0.f }, mfScalar{},
	mbEcoTool{ true }, mbRunEco{ false },
	mTerrain{ mnWidth, mnHeight },
	mAllCreatures{}, mTools{}, mHighlightQueue{}, mLogs{},
...
void Ecosystem::EcoSystem::Init(void) noexcept
{
	Data::MakeTools();
}
...
void Ecosystem::EcoSystem::UpdateCreatures(float _dt) const
{
	for (auto& c : mAllCreatures)
	{
		if (c->GetFlags() & Ecosystem::Creature::FLAG_DEAD)
			continue;
...
void Ecosystem::EcoSystem::HighlightGrid(unsigned short x, unsigned short y, unsigned int _col)
{
	mHighlightQueue.push(std::make_tuple(x, y, _col));
}

void Ecosystem::EcoSystem::HighlightGrid(unsigned x, unsigned y, unsigned int _col)
{
	mHighlightQueue.push(std::make_tuple(static_cast<unsigned short>(x), static_cast<unsigned short>(y), _col));
}

int Ecosystem::EcoSystem::GetWidth(void) const noexcept
{
	return mnWidth;
}

int Ecosystem::EcoSystem::GetHeight(void) const noexcept
{
	return mnHeight;
}
...
int Ecosystem::EcoSystem::GetGridVal(unsigned _x, unsigned _y) const noexcept
{
	...
	if (v>= 0 && static_cast<size_t>(v) >= mAllCreatures.size())
	{
		...
		fprintf(stderr, "Warning: Creature index %d out of range (size: %zu). Layer may need updating.\n", v, mAllCreatures.size());
		return -1;
	}
	return v;
}

float Ecosystem::EcoSystem::GetGrassVal(unsigned _x, unsigned _y) const noexcept
{
	if (_x > mnWidth || _y > mnHeight)
		return 0.f;
...
float Ecosystem::EcoSystem::GetGrassValA(unsigned _x, unsigned _y) const noexcept
{
	if (_x > mnWidth || _y > mnHeight)
		return 0.f;
...
const Ecosystem::Terrain& Ecosystem::EcoSystem::GetTerrain(void) const noexcept
{
	return mTerrain;
}
...
const std::deque<Ecosystem::Creature *>& Ecosystem::EcoSystem::GetAllCreatures(void) const noexcept
{
...
float Ecosystem::EcoSystem::Eat(const GridPos& _p, Creature* _predator)
{
	...
	if (sqrt((_p.x - static_cast<int>(x)) * (_p.x - static_cast<int>(x)) + (_p.y - static_cast<int>(y)) * (_p.y - static_cast<int>(y))) > 1.5f)
	{
		...
		fprintf(stderr, "Warning: Predator attempting to eat from distance > 1.5 units. Ignoring.\n");
		return 0.0f;
	}
...
std::vector<Ecosystem::GridPos> Ecosystem::EcoSystem::GetShortestPath(const GridPos& _src, const GridPos& _dest)
{
	return mTerrain.GetShortestPath(_src, _dest);
}

Ecosystem::GridPos Ecosystem::EcoSystem::GetBestGrassPos(const GridPos& _src, float _radiusLimit, float _minAlpha)
{
	return mTerrain.GetBestGrassPos(_src, _radiusLimit, _minAlpha);
}

Ecosystem::GridPos Ecosystem::EcoSystem::GetEmptyNeighbour(const GridPos& _src)
{
	return mTerrain.GetEmptyNeighbour(_src);
}

std::pair<float, float> Ecosystem::EcoSystem::GetScreenPos(const GridPos& _p) const noexcept
{
	ImRect bounds{ ImVec2{ 6.f, mfTitleBarSize + 6.f }, ImVec2{ static_cast<float>(mnWindowX) - 6.f, static_cast<float>(mnWindowY) - 6.f} };
...
float Ecosystem::EcoSystem::GetScalar(void) const noexcept
{
	return mfScalar;
}

const std::vector<std::deque<float>>& Ecosystem::EcoSystem::GetLogs(void) const noexcept
{
	return mLogs;
}

void Ecosystem::EcoSystem::ReturnEnergyToMap(float _v, const GridPos& _p) noexcept
{
