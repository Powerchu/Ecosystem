...
Ecosystem::GridPos::GridPos(int _x, int _y) noexcept
	: x{ _x }, y{ _y }
{}
...
Ecosystem::Terrain::Terrain(unsigned _x, unsigned _y)
	: mSpaceLayer{}, mGrassLayer{}, mFertilizerLayer{}, mNodeLayer{},
	mGrassLayerRate{}, mFertilizerRate{},
	mGrassThresh{}, mFertilizerThresh{},
	mnWidth{ _x }, mnHeight{ _y }
{
}

void Ecosystem::Terrain::Init(float _iga, float _igl, float _igh, unsigned _w, unsigned _h, float _grl, float _grh, float _gm, float _fm, float _frl, float _frh) noexcept
{
...
const Ecosystem::Map<int>& Ecosystem::Terrain::GetSpaceLayer(void) const noexcept
{
	return mSpaceLayer;
}
...
const Ecosystem::Map<float>& Ecosystem::Terrain::GetGrassLayer(void) const noexcept
{
	return mGrassLayer;
}
...
const Ecosystem::Map<float>& Ecosystem::Terrain::GetFertilizerLayer(void) const noexcept
{
...
const Ecosystem::Map<float>& Ecosystem::Terrain::GetGrassLayerRate(void) const noexcept
{
...
const Ecosystem::Map<std::pair<float, float>>& Ecosystem::Terrain::GetGrassLayerThresh(void) const noexcept
{
	return mGrassThresh;
}

Ecosystem::Map<std::pair<float, float>>& Ecosystem::Terrain::GetGrassLayerThresh(void) noexcept
{
	return mGrassThresh;
}

const Ecosystem::Map<std::pair<float, float>>& Ecosystem::Terrain::GetFertilizerLayerThresh(void) const noexcept
{
	return mFertilizerThresh;
}

Ecosystem::Map<std::pair<float, float>>& Ecosystem::Terrain::GetFertilizerLayerThresh(void) noexcept
{
	return mFertilizerThresh;
}
...
float Ecosystem::Terrain::ConsumeGrass(unsigned _x, unsigned _y, float _val) noexcept
{
...
unsigned int Ecosystem::Terrain::GetGrassColor(unsigned _x, unsigned _y) const noexcept
{
...
std::vector<Ecosystem::GridPos> Ecosystem::Terrain::GetShortestPath(const GridPos& _src, const GridPos& _dest) noexcept
{
	...
	while (!q.empty())
	{
			...
			if (q.size() > 1000)
		{
			...
			fprintf(stderr, "Warning: Pathfinding queue exceeded 1000 nodes. Aborting search.\n");
...
Ecosystem::GridPos Ecosystem::Terrain::GetBestGrassPos(const GridPos& _src, float _limit, float _minAlpha) noexcept
{
	for (auto& row : mNodeLayer)
	{
		for (auto& col : row)
...
std::vector<Ecosystem::Node*> Ecosystem::Terrain::GetNeighbours(const GridPos& _src, const GridPos& _dest, float _curT, Node * _prev) noexcept
{
	std::vector<Ecosystem::Node*> result;
	result.reserve(8);
	for (int j = -1; j < 2; ++j)
...
Ecosystem::GridPos Ecosystem::Terrain::GetEmptyNeighbour(const GridPos& _src) noexcept
{
	for (int j = -1; j < 2; ++j)
	{
		for (int i = -1; i < 2; ++i)
...
Ecosystem::GridPos Ecosystem::Terrain::GetLowestGrass(const GridPos& _src) noexcept
{
