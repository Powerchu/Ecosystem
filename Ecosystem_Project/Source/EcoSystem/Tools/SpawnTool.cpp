  ...
  if (ImGui::CollapsingHeader("Multiple")) {
    ...
    if (ImGui::Button("Batch Spawn", ImVec2{80, 30})) {
      ...
      for (int i = 0; i < mnSpawnCount; ++i) {
        std::uniform_int_distribution<unsigned short> distX(
            0,
            static_cast<unsigned short>(EcoSystem::GetInst().GetWidth()) - 1);
        std::uniform_int_distribution<unsigned short> distY(
            0,
            static_cast<unsigned short>(EcoSystem::GetInst().GetHeight()) - 1);
