...
void Ecosystem::ViewTool::Render(void) noexcept {
  ...
  for (unsigned i = 0; i < cs.size(); ++i) {
    ...
    if (ImGui::IsItemHovered()) {
      ...
      EcoSystem::GetInst().HighlightGrid(
          x, y, ImGui::GetColorU32(ImVec4{1.f, 1.f, 1.f, 1.f}));
      ...
      ImGui::Text("Mass : %f / %f", cs[i]->GetEnergy().first,
                  cs[i]->GetEnergy().second);
  ...
  for (unsigned y = 0; y < grid.size(); ++y) {
    for (unsigned x = 0; x < grid[y].size(); ++x) {
      ...
      if (ImGui::IsItemHovered()) {
        ...
        ImGui::Text("Thresh : %f / %f",
                    terrain.GetGrassLayerThresh()[y][x].first,
                    terrain.GetGrassLayerThresh()[y][x].second);
        ImGui::Unindent(indent);
        ImGui::Text("Fertilizer");
        ImGui::Indent(indent);
        ImGui::Text("Val    : %f", terrain.GetFertilizerLayer()[y][x]);
        ImGui::Text("Thresh : %f / %f",
                    terrain.GetFertilizerLayerThresh()[y][x].first,
                    terrain.GetFertilizerLayerThresh()[y][x].second);
        ImGui::Unindent(indent);
