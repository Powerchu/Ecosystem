...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis,
  ...
  ImRect bb = bb_frame;
  bb.Expand(ImVec2(
      -ImClamp((float)(int)((bb_frame_width - 2.0f) * 0.5f), 0.0f, 3.0f),
  ...
  // Render
  window->DrawList->AddRectFilled(bb_frame.Min, bb_frame.Max,
...
void ImGui::Scrollbar(ImGuiAxis axis) {
  ...
  if (axis == ImGuiAxis_X) {
    ...
    bb.Max = ImVec2(window->InnerMainRect.Max.x,
                    outer_rect.Max.y - window->WindowBorderSize);
    rounding_corners |= ImDrawCornerFlags_BotLeft;
  } else {
    bb.Min = ImVec2(window->InnerMainRect.Max.x, window->InnerMainRect.Min.y);
    bb.Max = ImVec2(outer_rect.Max.x - window->WindowBorderSize,
  ...
  ScrollbarEx(bb, id, axis, &window->Scroll[axis],
              window->SizeFull[axis] - other_scrollbar_size,
              window->SizeContents[axis], rounding_corners);
}
...
bool ImGui::Checkbox(const char* label, bool* v) {
  ...
  if (*v) {
    ...
    RenderCheckMark(check_bb.Min + ImVec2(pad, pad),
                    GetColorU32(ImGuiCol_CheckMark), square_sz - pad * 2.0f);
  }
...
void ImGui::ShrinkWidths(ImGuiShrinkWidthItem* items, int count,
  ...
  while (width_excess > 0.0f && count_same_width < count) {
    while (count_same_width < count &&
      ...
      count_same_width++;
    float width_to_remove_per_item_max =
...
int ImGui::DataTypeFormatString(char* buf, int buf_size,
                                ImGuiDataType data_type, const void* data_ptr,
                                const char* format) {
...
bool ImGui::DragBehaviorT(ImGuiDataType data_type, TYPE* v, float v_speed,
  ...
  const bool is_power =
      (power != 1.0f && is_decimal && has_min_max && (v_max - v_min < FLT_MAX));
  ...
  switch (data_type) {
    case ImGuiDataType_S8: {
      ...
      bool r = SliderBehaviorT<ImS32, ImS32, float>(
          bb, id, ImGuiDataType_S32, &v32, *(const ImS8*)v_min,
    ...
    case ImGuiDataType_U8: {
      ...
      bool r = SliderBehaviorT<ImU32, ImS32, float>(
          bb, id, ImGuiDataType_U32, &v32, *(const ImU8*)v_min,
    ...
    case ImGuiDataType_S16: {
      ImS32 v32 = (ImS32) * (ImS16*)v;
      bool r = SliderBehaviorT<ImS32, ImS32, float>(
          bb, id, ImGuiDataType_S32, &v32, *(const ImS16*)v_min,
    ...
    case ImGuiDataType_U16: {
      ImU32 v32 = (ImU32) * (ImU16*)v;
      bool r = SliderBehaviorT<ImU32, ImS32, float>(
          bb, id, ImGuiDataType_U32, &v32, *(const ImU16*)v_min,
...
bool ImGui::TempInputTextScalar(const ImRect& bb, ImGuiID id, const char* label,
                                ImGuiDataType data_type, void* data_ptr,
                                const char* format) {
  ImGuiContext& g = *GImGui;
  ...
  bool value_changed = InputTextEx(label, NULL, data_buf,
                                   IM_ARRAYSIZE(data_buf), bb.GetSize(), flags);
  if (init) {
...
bool ImGui::InputScalar(const char* label, ImGuiDataType data_type,
  ...
  flags |=
      ImGuiInputTextFlags_NoMarkEdited;  // We call MarkItemEdited() ourselve by
                                         // comparing the actual data rather
                                         // than the string.
  ...
  if (step != NULL) {
    ...
    if (InputText("", buf, IM_ARRAYSIZE(buf),
      ...
      value_changed = DataTypeApplyOpFromText(
          buf, g.InputTextState.InitialTextA.Data, data_type, data_ptr, format);
      ...
      if (is_multiline) {
        ...
        draw_window->DC.CursorPos.y +=
            (draw_window->Scroll.y -
             scroll_y);  // Manipulate cursor pos immediately avoid a frame of
                         // lag
    ...
    if (render_selection) {
      ...
      for (const ImWchar* p = text_selected_begin; p < text_selected_end;) {
        ...
        if (rect_pos.y < clip_rect.y) {
          ...
          // are 16-bits p = p ? p + 1 : text_selected_end;
  ...
  if (!(flags & ImGuiColorEditFlags_NoSmallPreview)) {
    ...
    if (BeginPopup("picker")) {
      ...
      ImGuiColorEditFlags picker_flags =
          ...
          ImGuiColorEditFlags_DisplayHSV | ImGuiColorEditFlags_NoLabel |
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags,
  ...
  ImRect frame_bb =
      ImRect(window->DC.CursorPos,
             ImVec2(GetWorkRectMax().x, window->DC.CursorPos.y + frame_height));
  if (display_frame) {
...
void ImGui::TreePop() {
  ...
  if (g.NavMoveDir == ImGuiDir_Left && g.NavWindow == window &&
    ...
    if (g.NavIdIsAlive && (window->DC.TreeStoreMayJumpToParentOnPop &
...
bool ImGui::Selectable(const char* label, bool selected,
  ...
  // Selectables are tightly packed together so we extend the box to cover
...
void ImGui::EndMenuBar() {
  ...
  if (NavMoveRequestButNoResultYet() &&
    ...
    if (nav_earliest_child->ParentWindow == window &&
      ...
      FocusWindow(window);
      SetNavIDWithRectRel(window->NavLastIds[layer], layer,
...
bool ImGui::BeginMenu(const char* label, bool enabled) {
  ...
  // to be different! It is choosen by FindBestWindowPosForPopup(). e.g. Menus
        ...
        (float)(int)(style.ItemSpacing.x *
                     (-1.0f +
                      0.5f));  // -1 spacing to compensate the spacing added
                               // when Selectable() did a SameLine(). It would
                               // also work to call SameLine() ourselves after
                               // the PopStyleVar().
  } else {
    // Menu inside a menu
...
bool ImGui::BeginTabBar(const char* str_id, ImGuiTabBarFlags flags) {
  ...
  ImRect tab_bar_bb =
      ImRect(window->DC.CursorPos.x, window->DC.CursorPos.y,
...
bool ImGui::BeginTabBarEx(ImGuiTabBar* tab_bar, const ImRect& tab_bar_bb,
  ...
  const ImU32 col = GetColorU32(
      (flags & ImGuiTabBarFlags_IsFocused) ? ImGuiCol_TabActive : ImGuiCol_Tab);
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar) {
  ...
  for (int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++) {
    ...
    tab->WidthContents =
        TabItemCalcSize(tab_name, (tab->Flags & ImGuiTabItemFlags_NoCloseButton)
                                      ? false
                                      : true)
            .x;
...
bool ImGui::TabItemLabelAndCloseButton(ImDrawList* draw_list, const ImRect& bb,
  ...
  // text_pixel_clip_bb and !close_button_visible makes it tricky to abstract at
  // the moment
  const char* label_display_end = FindRenderedTextEnd(label);
  if (label_size.x > text_ellipsis_clip_bb.GetWidth()) {
    ...
    float label_size_clipped_x =
        g.Font
            ->CalcTextSizeA(
                g.FontSize,
                text_ellipsis_clip_bb.GetWidth() - ellipsis_width + 1.0f, 0.0f,
                label, label_display_end, &label_end)
            .x;
    if (label_end == label &&
        label_end <
                                ...
                                // there's no room for character + ellipsis
    {
      ...
      label_size_clipped_x =
          g.Font->CalcTextSizeA(g.FontSize, FLT_MAX, 0.0f, label, label_end).x;
    }
