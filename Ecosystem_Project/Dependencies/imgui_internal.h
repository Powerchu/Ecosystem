...
IMGUI_API ImU32 ImHashStr(const char* data, size_t data_size = 0,
                          ImU32 seed = 0);
...
enum ImGuiButtonFlags_ {
  ...
  ImGuiButtonFlags_NoHoveredOnNav =
      1 << 14  // don't report as hovered when navigated on
};
...
enum ImGuiSelectableFlagsPrivate_ {
  ...
  ImGuiSelectableFlags_DrawFillAvailWidth =
      1 << 13,  // FIXME: We may be able to remove this (added in 6251d379 for
                // menus)
...
enum ImGuiItemStatusFlags_ {
  ...
  ImGuiItemStatusFlags_HasDeactivated =
      1 << 4,  // Set if the widget/group is able to provide data for the
...
struct IMGUI_API ImGuiInputTextState {
  ...
  int CurLenW,
      CurLenA;  // we need to maintain our buffer length in both UTF-8 and wchar
                // format. UTF-8 len is valid even if TextA is not.
...
struct ImGuiContext {
  ...
  ImGuiID NavJustMovedToMultiSelectScopeId;  // Just navigated to this select
  ...
  // Tabbing system (older than Nav, active even if Nav is disabled. FIXME-NAV:
  // This needs a redesign!)
  ImGuiWindow* FocusRequestCurrWindow;  //
  ImGuiWindow* FocusRequestNextWindow;  //
  int FocusRequestCurrCounterAll;  // Any item being requested for focus, stored
                                   // as an index (we on layout to be stable
                                   // between the frame pressing TAB and the
                                   // next frame, semi-ouch)
  int FocusRequestCurrCounterTab;  // Tab item being requested for focus, stored
                                   // as an index
  ...
  ImGuiID TempInputTextId;  // Temporary text input when CTRL+clicking on a
                            // slider, etc.
  ImGuiColorEditFlags
  ...
  // [This is unused in this branch, but left here to facilitate merging/syncing
  // multiple branches]
  ...
  ImGuiContext(ImFontAtlas* shared_font_atlas)
      : BackgroundDrawList(NULL), ForegroundDrawList(NULL) {
    Initialized = false;
    ...
    NavId = NavActivateId = NavActivateDownId = NavActivatePressedId =
        NavInputId = 0;
    NavJustTabbedId = NavJustMovedToId = NavJustMovedToMultiSelectScopeId =
        NavNextActivateId = 0;
...
struct IMGUI_API ImGuiWindowTempData {
  ...
  int TreeDepth;
  ImU32 TreeStoreMayJumpToParentOnPop;  // Store a copy of !g.NavIdIsAlive for
  ...
  int FocusCounterAll;   // Counter for focus/tabbing system. Start at -1 and
                         // increase as assigned via FocusableItemRegister()
                         // (FIXME-NAV: Needs redesign)
  int FocusCounterTab;   // (same, but only count widgets which you can Tab
                         // through)
...
struct IMGUI_API ImGuiWindow {
  ...
  ImRect InnerWorkRectClipped;  // == InnerMainRect minus WindowPadding.x,
                                // clipped within viewport or parent clip rect.
  ImRect
...
enum ImGuiTabItemFlagsPrivate_ {
  ImGuiTabItemFlags_NoCloseButton =
      1 << 20  // Store whether p_open is set or not, which we need to recompute
               // WidthContents during layout.
};
...
IMGUI_API void RenderPixelEllipsis(ImDrawList* draw_list, ImVec2 pos, int count,
                                   ImU32 col);
...
IMGUI_API bool DragBehaviorT(ImGuiDataType data_type, T* v, float v_speed,
                             T v_min, T v_max, const char* format, float power,
                             ImGuiDragFlags flags);
template <typename T, typename SIGNED_T, typename FLOAT_T>
...
IMGUI_API bool InputTextEx(const char* label, const char* hint, char* buf,
                           ...
                           void* user_data = NULL);
IMGUI_API bool TempInputTextScalar(const ImRect& bb, ImGuiID id,
