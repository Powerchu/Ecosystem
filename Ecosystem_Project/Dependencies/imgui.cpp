...
application?
  ...
  - Why are multiple widgets reacting when I interact with a single one? How can
I have multiple widgets with the same label or with an empty label? A primer on
...
/*
 ...
 A: I occasionally tag Releases (https://github.com/ocornut/imgui/releases) but
 it is generally safe and recommended to sync to master/latest. The library is
 fairly stable and regressions tend to be fixed fast when reported. You may also
 peak at the 'docking' branch which includes:
    - Docking/Merging features (https://github.com/ocornut/imgui/issues/2109)
    - Multi-viewport features (https://github.com/ocornut/imgui/issues/1542)
    Many projects are using this branch and it is kept in sync with master
 regularly.

 Q: Who uses Dear ImGui?
 A: See "Quotes" (https://github.com/ocornut/imgui/wiki/Quotes) and
    "Software using Dear ImGui"
 (https://github.com/ocornut/imgui/wiki/Software-using-dear-imgui) Wiki pages
    for a list of games/software which are publicly known to use dear imgui.
 Please add yours if you can!

 Q: Why the odd dual naming, "Dear ImGui" vs "ImGui"?
 A: The library started its life as "ImGui" due to the fact that I didn't give
 it a proper name when when I released 1.0, and had no particular expectation
 that it would take off. However, the term IMGUI (immediate-mode graphical user
 interface) was coined before and is being used in variety of other situations
 (e.g. Unity uses it own implementation of the IMGUI paradigm). To reduce the
 ambiguity without affecting existing code bases, I have decided on an
 alternate, longer name "Dear ImGui" that people can use to refer to this
 specific library. Please try to refer to this library as "Dear ImGui".
 ...
 pointer or integer into it by casting your value to ImTextureID / void*, and
 vice-versa. Because both end-points (user code and rendering function) are
 under your control, you know exactly what is stored inside the ImTextureID /
 void*. Examples:
 ...
 passing glm::vec2 to ImGui functions instead of ImVec2.
 ...
 GetWindowDrawList() and draw to it in any way you like.
    - You can call ImGui::GetBackgroundDrawList() or
 ImGui::GetForegroundDrawList() and use those draw list to display contents
 behind or over every other imgui windows (one bg/fg drawlist per viewport).
...
ImGuiStyle::ImGuiStyle() {
  ...
  DisplayWindowPadding = ImVec2(
      19,
      19);  // Window position are clamped to be visible within the display area
            // by at least this amount. Only applies to regular windows.
...
static void SetCursorPosYAndSetupDummyPrevLine(float pos_y, float line_height) {
  ...
  if (window->DC.CurrentColumns)
    window->DC.CurrentColumns->LineMinY =
        window->DC.CursorPos
...
void ImGui::ItemSize(const ImVec2& size, float text_offset_y) {
  ...
  const float text_base_offset =
      ImMax(window->DC.CurrLineTextBaseOffset, text_offset_y);
...
bool ImGui::FocusableItemRegister(ImGuiWindow* window, ImGuiID id) {
  ...
  if (g.FocusRequestCurrWindow == window) {
    ...
    if (is_tab_stop &&
        window->DC.FocusCounterTab == g.FocusRequestCurrCounterTab) {
...
// pragma, etc. your user code may see different structures thanwhat imgui.cpp
...
void ImGui::UpdateMouseMovingWindowEndFrame() {
  ...
  if (g.IO.MouseClicked[1]) {
    // Find the top-most window between HoveredWindow and the front most Modal
...
void ImGui::UpdateMouseWheel() {
  ...
  const bool scroll_allowed =
      !(window->Flags & ImGuiWindowFlags_NoScrollWithMouse) &&
      !(window->Flags & ImGuiWindowFlags_NoMouseInputs);
  if (scroll_allowed && (g.IO.MouseWheel != 0.0f || g.IO.MouseWheelH != 0.0f) &&
      !g.IO.KeyCtrl) {
    ImVec2 max_step =
        (window->ContentsRegionRect.GetSize() + window->WindowPadding * 2.0f) *
        0.67f;

    // Vertical Mouse Wheel Scrolling (hold Shift to scroll horizontally)
    if (g.IO.MouseWheel != 0.0f && !g.IO.KeyShift) {
      float scroll_step =
          ImFloor(ImMin(5 * window->CalcFontSize(), max_step.y));
      SetWindowScrollY(window,
                       window->Scroll.y - g.IO.MouseWheel * scroll_step);
    } else if (g.IO.MouseWheel != 0.0f && g.IO.KeyShift) {
      float scroll_step =
          ImFloor(ImMin(2 * window->CalcFontSize(), max_step.x));
      SetWindowScrollX(window,
                       window->Scroll.x - g.IO.MouseWheel * scroll_step);
    }
    ...
    if (g.IO.MouseWheelH != 0.0f && !g.IO.KeyShift) {
      float scroll_step =
          ImFloor(ImMin(2 * window->CalcFontSize(), max_step.x));
      SetWindowScrollX(window,
                       window->Scroll.x - g.IO.MouseWheelH * scroll_step);
    }
  }
}
...
void ImGui::NewFrame() {
  ...
  if (g.ActiveId == 0 && g.FocusTabPressed) {
    // Note that SetKeyboardFocusHere() sets the Next fields mid-frame. To be
    // consistent we also manipulate the Next fields even, even though they will
    // be turned into Curr fields by the code below.
  ...
  if (g.FocusRequestNextWindow != NULL) {
    ...
    if (g.FocusRequestNextCounterAll != INT_MAX &&
        window->DC.FocusCounterAll != -1)
      g.FocusRequestCurrCounterAll = ImModPositive(
          g.FocusRequestNextCounterAll, window->DC.FocusCounterAll + 1);
    if (g.FocusRequestNextCounterTab != INT_MAX &&
        window->DC.FocusCounterTab != -1)
      g.FocusRequestCurrCounterTab = ImModPositive(
          g.FocusRequestNextCounterTab, window->DC.FocusCounterTab + 1);
    g.FocusRequestNextWindow = NULL;
...
static void FindHoveredWindow() {
  ...
  for (int i = g.Windows.Size - 1; i >= 0; i--) {
    ...
    // Those seemingly unnecessary extra tests are because the code here is a
    // little different in viewport/docking branches.
...
bool ImGui::IsMouseClicked(int button, bool repeat) {
  ...
  if (repeat && t > g.IO.KeyRepeatDelay) {
    // FIXME: 2019/05/03: Our old repeat code was wrong here and led to doubling
    // the repeat rate, which made it an ok rate for repeat on mouse hold.
    int amount = CalcTypematicPressedRepeatAmount(
        t, t - g.IO.DeltaTime, g.IO.KeyRepeatDelay, g.IO.KeyRepeatRate * 0.5f);
...
static ImRect GetResizeBorderRect(ImGuiWindow* window, int border_n,
  ...
  if (border_n == 1)
    return ImRect(rect.Max.x - thickness, rect.Min.y + perp_padding,
                  rect.Max.x + thickness, rect.Max.y - perp_padding);  // Right
  if (border_n == 2)
    return ImRect(rect.Min.x + perp_padding, rect.Max.y - thickness,
                  rect.Max.x - perp_padding, rect.Max.y + thickness);  // Bottom
  if (border_n == 3)
    return ImRect(rect.Min.x - thickness, rect.Min.y + perp_padding,
                  rect.Min.x + thickness, rect.Max.y - perp_padding);  // Left
  IM_ASSERT(0);
  return ImRect();
}
    ...
    if (held && g.IO.MouseDoubleClicked[0] && resize_grip_n == 0) {
      ...
      ImVec2 corner_target =
          ...
          ImLerp(grip.InnerDir * grip_hover_outer_size,
                 ...
                 grip.CornerPosN);  // Corner of the window corresponding to our
                                    // corner grip
      CalcResizePosSizeFromAnyCorner(window, corner_target, grip.CornerPosN,
...
static inline void ClampWindowRect(ImGuiWindow* window, const ImRect& rect,
  ...
  ImVec2 size_for_clamping =
      (g.IO.ConfigWindowsMoveFromTitleBarOnly &&
       !(window->Flags & ImGuiWindowFlags_NoTitleBar))
          ...
          : window->Size;
  window->Pos = ImMin(rect.Max - padding, ImMax(window->Pos + size_for_clamping,
                                                rect.Min + padding) -
                                              size_for_clamping);
}
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags) {
  ...
  if (first_begin_of_the_frame) {
    ...
    // partly faulty, but some BeginChild() patterns relies on it. NB:
    // WindowBorderSize is included in WindowPadding _and_ ScrollbarSizes so we
    // need to cancel one out.
    ...
    window->ContentsRegionRect.Max.x =
        ...
        (window->SizeContentsExplicit.x != 0.0f
             ...
             : (window->Size.x - window->ScrollbarSizes.x +
                ImMin(window->ScrollbarSizes.x, window->WindowBorderSize)));
    window->ContentsRegionRect.Max.y =
        window->Pos.y - window->Scroll.y - window->WindowPadding.y +
        (window->SizeContentsExplicit.y != 0.0f
             ? window->SizeContentsExplicit.y
             : (window->Size.y - window->ScrollbarSizes.y +
                ImMin(window->ScrollbarSizes.y, window->WindowBorderSize)));
    ...
    window->InnerMainRect.Max.x =
        ...
        ImMax(window->ScrollbarSizes.x, window->WindowBorderSize);
    window->InnerMainRect.Max.y =
        window->Pos.y + window->Size.y -
        ImMax(window->ScrollbarSizes.y, window->WindowBorderSize);
    ...
    // Inner work/clipping rectangle will extend a little bit outside the work
    // region. This is to allow e.g. Selectable or CollapsingHeader or some
    // separators to cover that space. Force round operator last to ensure that
    // e.g. (int)(max.x-min.x) in user's render code produce correct result.
    window->InnerWorkRect.Min.x =
  ...
  PushClipRect(window->InnerWorkRectClipped.Min,
               window->InnerWorkRectClipped.Max, true);
...
ImVec2 ImGui::CalcItemSize(ImVec2 size, float default_w, float default_h) {
  ...
  ImVec2 region_max;
  if (size.x < 0.0f || size.y < 0.0f) region_max = GetWorkRectMax();
...
// FIXME: This is in window space (not screen space!). We should try to obsolete
// all those functions.
ImVec2 ImGui::GetContentRegionMax() {
  ImGuiWindow* window = GImGui->CurrentWindow;
  ImVec2 mx = window->ContentsRegionRect.Max - window->Pos;
  if (window->DC.CurrentColumns)
    mx.x = GetColumnOffset(window->DC.CurrentColumns->Current + 1) -
...
ImVec2 ImGui::GetWorkRectMax() {
  ...
  if (window->DC.CurrentColumns)
    mx.x = window->Pos.x +
           GetColumnOffset(window->DC.CurrentColumns->Current + 1) -
           window->WindowPadding.x;
  return mx;
}
...
void ImGui::ClosePopupsOverWindow(ImGuiWindow* ref_window,
  ...
  if (ref_window) {
    ...
    for (; popup_count_to_keep < g.OpenPopupStack.Size; popup_count_to_keep++) {
      ...
      // Trim the stack when popups are not direct descendant of the reference
...
static void ImGui::NavProcessItem(ImGuiWindow* window, const ImRect& nav_bb,
  ...
  if (g.NavId == id) {
    g.NavWindow =
        window;  // Always refresh g.NavWindow, because some operations such as
  ...
  if (g.NavDisableHighlight || !g.NavDisableMouseHover || !g.NavWindow) {
    ...
    return ImFloor(ImClamp(
        ...
        visible_rect.Max));  // ImFloor() is important because non-integer mouse
                             // position application in back-end might be lossy
                             // and result in undesirable non-zero delta.
  }
}
...
#undef NAV_MAP_KEY
  ...
  IM_ASSERT(
      !g.NavScoringRectScreen
           .IsInverted());  // Ensure if we have a finite, non-inverted bounding
                            // box here will allows us to remove extraneous
    ...
    if (page_up_held != page_down_held)  // If either (not both) are pressed
    {
      if (window->DC.NavLayerActiveMask == 0x00 && window->DC.NavHasScroll) {
        ...
        if (IsKeyPressed(g.IO.KeyMap[ImGuiKey_PageUp], true))
          SetWindowScrollY(
              window, window->Scroll.y - window->InnerMainRect.GetHeight());
        ...
        const float page_offset_y = ImMax(
            0.0f, window->InnerMainRect.GetHeight() -
...
static void ImGui::NavUpdateWindowing() {
  ...
  if (apply_toggle_layer && g.NavWindow) {
    ...
    // When entering a regular menu bar with the Alt key, we always reinitialize
    // the navigation ID.
...
void ImGui::NextColumn() {
  ...
  PushColumnClipRect(
      columns->Current);  // FIXME-COLUMNS: Could it be an overwrite?
  PushItemWidth(GetColumnWidth() *
                0.65f);  // FIXME-COLUMNS: Move on columns setup
}
...
static float OffsetNormToPixels(const ImGuiColumns* columns,
                                float offset_norm) {
...
void ImGui::PushColumnsBackground() {
  ...
  [[maybe_unused]] int cmd_size = window->DrawList->CmdBuffer.Size;
  PushClipRect(columns->HostClipRect.Min, columns->HostClipRect.Max, false);
  IM_ASSERT(
      cmd_size ==
      window->DrawList->CmdBuffer.Size);  // Being in channel 0 this should not
                                          // have created an ImDrawCmd
}
...
void ImGui::BeginColumns(const char* str_id, int columns_count,
  ...
  const float content_region_width =
      (window->SizeContentsExplicit.x != 0.0f)
          ? (window->SizeContentsExplicit.x)
          : (window->InnerWorkRect.Max.x - window->Pos.x);
  columns->OffMinX =
      window->DC.Indent.x - g.Style.ItemSpacing.x;  // Lock our horizontal range
  columns->OffMaxX =
      ImMax(content_region_width - window->Scroll.x, columns->OffMinX + 1.0f);
...
void ImGui::LogRenderedText(const ImVec2* ref_pos, const char* text,
  ...
  for (;;) {
    ...
    if (!is_last_line || (line_start != line_end)) {
      ...
      // An empty "" string at a different Y position should output a carriage
      // return.
      LogText(IM_NEWLINE);
      break;
    }
...
static void SetClipboardTextFn_DefaultImpl(void*, const char* text) {
  ...
  ::CloseClipboard();
    ...
    !defined(IMGUI_DISABLE_OSX_FUNCTIONS)
...
static const char* GetClipboardTextFn_DefaultImpl(void*) {
  ...
  for (int i = 0; i < static_cast<int>(item_count); i++) {
...
void ImGui::ShowMetricsWindow(bool* p_open) {
    ...
    static void NodeDrawList(ImGuiWindow* window, ImDrawList* draw_list,
      ...
      if (draw_list == ImGui::GetWindowDrawList()) {
        ...
        ImGui::TextColored(
            ...
            "CURRENTLY APPENDING");  // Can't display stats for active draw
                                     // list! (we don't have the data
                                     // double-buffered)
      ...
      for (const ImDrawCmd* pcmd = draw_list->CmdBuffer.begin();
        ...
        bool pcmd_node_open = ImGui::TreeNode(
            ...
            (void*)(intptr_t)pcmd->TextureId, pcmd->ClipRect.x,
        ...
        if (show_drawcmd_clip_rects && fg_draw_list && ImGui::IsItemHovered()) {
          ...
          clip_rect.Floor();
          fg_draw_list->AddRect(clip_rect.Min, clip_rect.Max,
                                IM_COL32(255, 255, 0, 255));
          vtxs_rect.Floor();
          fg_draw_list->AddRect(vtxs_rect.Min, vtxs_rect.Max,
          ...
          for (int prim = clipper.DisplayStart,
            ...
            for (int n = 0; n < 3; n++, idx_i++) {
              ...
              buf_p += ImFormatString(
                  ...
                  (n == 0) ? "idx" : "   ", idx_i, v.pos.x, v.pos.y, v.uv.x,
                  v.uv.y, v.col);
            }
            ...
            if (fg_draw_list && ImGui::IsItemHovered()) {
              ...
              fg_draw_list->Flags &=
                  ~ImDrawListFlags_AntiAliasedLines;  // Disable AA on triangle
                                                      ...
                                                      // and thin triangles.
              fg_draw_list->AddPolyline(triangles_pos, 3,
  ...
  if (ImGui::TreeNode("Tools")) {
    ...
    show_windows_rects |= ImGui::Combo("##rects_type", &show_windows_rect_type,
                                       "OuterRect\0"
                                       "OuterRectClipped\0"
                                       "InnerMainRect\0"
                                       "InnerWorkRect\0"
                                       "InnerWorkRectClipped\0"
                                       "ContentsRegionRect\0");
  ...
  if (show_windows_rects || show_windows_begin_order) {
    for (int n = 0; n < g.Windows.Size; n++) {
      ...
      if (show_windows_rects) {
        ImRect r;
        if (show_windows_rect_type == RT_OuterRect) {
          r = window->Rect();
        } else if (show_windows_rect_type == RT_OuterRectClipped) {
          r = window->OuterRectClipped;
        } else if (show_windows_rect_type == RT_InnerMainRect) {
          r = window->InnerMainRect;
        } else if (show_windows_rect_type == RT_InnerWorkRect) {
          r = window->InnerWorkRect;
        } else if (show_windows_rect_type == RT_InnerWorkRectClipped) {
          r = window->InnerWorkRectClipped;
        } else if (show_windows_rect_type == RT_ContentsRegionRect) {
          r = window->ContentsRegionRect;
        }
