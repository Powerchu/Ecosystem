...
static void ShowDemoWindowWidgets() {
    ...
    if (ImGui::TreeNode("Advanced, with Selectable nodes")) {
      ...
      for (int i = 0; i < 6; i++) {
        ...
        if (i < 3) {
          ...
          // leaf. Otherwise we can use BulletText() or
  ...
  if (ImGui::TreeNode("Images")) {
    ...
    // 'ID3D11ShaderResourceView*' pointer. The imgui_impl_glfw_gl3.cpp renderer
  ...
  if (ImGui::TreeNode("Text Input")) {
    ...
    if (ImGui::TreeNode("Resize Callback")) {
      ...
      Funcs::MyInputTextMultiline(
          "##MyStr", &my_str, ImVec2(-1.0f, ImGui::GetTextLineHeight() * 16));
...
// natural type maximum, hence the /2 below.
  ...
  if (ImGui::TreeNode("Querying Status (Active/Focused/Hovered etc.)")) {
    ...
    // functions. Note that the flags can be combined. (because BulletText is an
    // item itself and that would affect the output of IsItemHovered() we pass
    ...
    // Testing IsWindowFocused() function with its various flags. Note that the
    ...
    // Testing IsWindowHovered() function with its various flags. Note that the
    // flags can be combined.
    ImGui::BulletText(
        "IsWindowHovered() = %d\n"
...
static void ShowDemoWindowLayout() {
  ...
  if (ImGui::TreeNode("Groups")) {
    HelpMarker(
        ...
        "basically locks the horizontal position. EndGroup() bundles the whole "
        "group so that you can use functions such as IsItemHovered() on it.");
    ImGui::BeginGroup();
    {
      ImGui::BeginGroup();
      ImGui::Button("AAA");
  ...
  if (ImGui::TreeNode("Text Baseline Alignment")) {
    HelpMarker(
        ...
        "\"small\" widgets fit in less vertical spaces than lines with normal "
        "widgets.");
    ...
    ImGui::AlignTextToFramePadding();  // Vertically align text node a bit lower
                                       // so it'll be vertically centered with
                                       // upcoming widget. Otherwise you can use
  ...
  if (ImGui::BeginTabBar("##tabs", ImGuiTabBarFlags_None)) {
    if (ImGui::BeginTabItem("Sizes")) {
      ...
      ImGui::Combo("WindowMenuButtonPosition",
                   (int*)&style.WindowMenuButtonPosition, "Left\0Right\0");
    ...
    if (ImGui::BeginTabItem("Fonts")) {
                ...
                for (int n = 0; n < 256; n++) {
                  ...
                  if (glyph)
                    font->RenderChar(
                        ...
                        (ImWchar)(base +
                                  n));  // We use ImFont::RenderChar as a
                                        // shortcut because we don't have UTF-8
                                        // conversion functions available to
                                        // generate a string.
      ...
      if (ImGui::DragFloat("this window scale", &window_scale, 0.005f, 0.3f,
    ...
    if (ImGui::BeginTabItem("BG/FG draw lists")) {
      ...
      if (draw_bg)
        ImGui::GetBackgroundDrawList()->AddCircle(
            window_center, window_size.x * 0.6f, IM_COL32(255, 0, 0, 200), 32,
            10 + 4);
      if (draw_fg)
        ImGui::GetForegroundDrawList()->AddCircle(
            window_center, window_size.y * 0.6f, IM_COL32(0, 255, 0, 200), 32,
            10);
      ImGui::EndTabItem();
    }
