...
ImDrawChannel, ImDrawListFlags, ImDrawList, ImDrawData)
...
IMGUI_API void
EndFrame();  // ends the Dear ImGui frame. automatically called by Render(), you
             ...
             // at all!
IMGUI_API void
Render();  // ends the Dear ImGui frame, finalize the draw data. You can get
           // call GetDrawData() to obtain it and run your rendering function.
           // (Obsolete: this used to call io.RenderDrawListsFn(). Nowadays, we
           // allow and prefer calling your render function yourself.)
...
IMGUI_API void PushItemWidth(
    float item_width);  // set width of items for common large "item+label"
                        // widgets. >0.0f: width in pixels, <0.0f align xx
                        // pixels to the right of window (so -1.0f always align
                        // width to the right side). 0.0f = default to ~2/3 of
                        // windows width,
...
IMGUI_API void SetNextItemWidth(
    float item_width);  // set width of the _next_ common large "item+label"
                        ...
                        // to the right of window (so -1.0f always align width
                        // to the right side)
...
// Enumateration for ImGui::SetWindow***(), SetNextWindow***(), SetNextItem***()
...
struct ImGuiStyle {
  ...
  ImGuiDir WindowMenuButtonPosition;  // Side of the collapsing/docking button
                                      // in the title bar (left/right). Defaults
                                      // to ImGuiDir_Left.
...
struct ImGuiIO {
  ...
  const char* IniFilename;  // = "imgui.ini"    // Path to .ini file. Set NULL
                            // to disable automatic .ini loading/saving, if e.g.
                            // you want to manually load/save from memory.
  const char*
  ...
  bool ConfigWindowsMoveFromTitleBarOnly;  // = false       // [BETA] Set to
                                           ...
                                           // title bar. Windows without a title
                                           // bar are not affected.
  ...
  // displayed in About Window) + User data for back-end/wrappers to store their
  // own stuff.
  ...
  void* ImeWindowHandle;  // = NULL           // (Windows) Set this to your HWND
                          // to get automatic IME cursor positioning.
  ...
  bool MouseDownOwned[5];      // Track if button was clicked inside an imgui
  ...
  IMGUI_API void ScaleClipRects(
      const ImVec2&
          fb_scale);  // Helper to scale the ClipRect field of each ImDrawCmd.
                      // Use if your final output buffer is at a different scale
                      // than ImGui expects, or if there is a difference between
...
struct ImFontAtlas {
  ...
  IMGUI_API const ImWchar*
  GetGlyphRangesVietnamese();  // Default + Vietname characters
